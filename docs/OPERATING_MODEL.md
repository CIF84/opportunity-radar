...
| `output/credential_semantics_validation/*/aggregate_termination.json` | EXPERIMENT RESULT, sanitized/incomplete | Track terminal counts, private-evidence hashes, source-decay rationale, and limitations; never present it as final 50-case validation or rule precision |
| `output/credential_evidence_validation/*/manifest.json`, `blind_review.md`, `detailed_report.json` | CANONICAL_EVIDENCE / DERIVED_REPORT, private | Retain locally and exclude from Git because exact source wording, vacancy identity, and candidate substitution evidence are joined here |
| `data/credential_evidence_validation/*.jsonl` | CANONICAL_EVIDENCE, private | Append-only SPEC-021 judgments/replacements; exclude from Git and back up privately |
| `output/credential_evidence_validation/*/aggregate_summary.json` | EXPERIMENT RESULT, sanitized preparation receipt | Track only population/sample aggregates, fingerprints, source-currentness aggregate, limitations, and zero-call/read-only proof; never include vacancy text, identity, or human notes |
| `output/credential_evidence_validation/*/aggregate_result.json` | EXPERIMENT RESULT, sanitized completed result | Track only completed label/gap/pattern aggregates, class coverage, diagnostic architecture/replay ranges, fingerprints, limitations, and zero-call/read-only proof; never include vacancy text, identity, or human notes |
